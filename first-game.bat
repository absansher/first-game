...
:: ABSANSHER HACKERS GROUP 2018
set ^"LF=^

^" Above empty line is required - do not remove

:: DASTURIMIZGA BAHO BERING
for /f %%A in ('copy /Z "%~dpf0" nul') do set "CR=%%A"

:: DASTURCHILAR abdurasul,sanjar,sherzod,Muzrobjon aka
...
set screen=!screen!TEZLIK= !Difficulty! !replay!!LF!O'SISH DARAJASI= !growth!   ENG YAXSHI NATIJA = !hi!!LF!BALL= !score!  VAQT = !m!:!s!%\n%
if defined replay if not defined replayFinished (%\n%
  set screen=!screen!!LF!!LF!O'YINNI TUGATISH UCHUN "ENTER"NI BOSING%\n%
)%\n%
%cls%^&echo(!screen!
...
for /l %%. in () do (
  ...
  if !tDiff! geq !delay! (
    ...
    if /i !key! equ !pause! (
      ...
      %=== pause game ===%
      echo(
      call :ask "OYINNI DAVOM ETTIRISH..."
      %ClearPrev%
      %sendCmd% go
      for /f "tokens=1-4 delims=:.," %%a in ("!time: =0!") do set /a "t2=(((1%%a*60)+1%%b)*60+1%%c)*100+1%%d-36610100, tDiff=t2-t1"
      if !tDiff! lss 0 set /a tDiff+=24*60*60*100
      ...
      (%test% !X! !Y! playerSpace) && (
        ...
        if defined newHi (
          echo(
          echo YANGI REKORD - TABRIKLAYMIZ !
...
cls
set "loadAvailable="
echo                 O'SISH DARAJASI = !growth!
echo(
echo                  O'YIN MENYUSI
echo(
echo        N - YANGI O'YIN
if defined replayAvailable echo        R - DAVOM ETTIRISH
if defined saveAvailable   echo        S - SAQLASH
if exist *.snake.txt       echo        L - SAQLANGAN O'YINNI O'YNASH
...
if /i !key! equ S (
  if defined replayAvailable (
    ...
    call :ask "NATIJANI KIRITING#:" !hiAvailable!
  )
  echo !key!
  if /i !key! equ P (set "src=!gameLog!") else set "src=%hiFile%!key!.txt"
  call :getString "FAYL NOMI" file 20
  copy "!src!" "!file!.snake.txt"
  call :ask "DAVOM ETTIRISH UCHUN BOSING..."
)
if /i !key! equ L (
...
if !moveKeys! equ 4 set "keys=!keys! U D"
                    ...
                    echo(
                    echo   O'YIN TURI    = !moveKeys! keys
                    echo(
                    echo   CHAP          = !left!
                    echo   O'NG          = !right!
if !moveKeys! equ 4 echo   TEPAGA        = !up!
if !moveKeys! equ 4 echo   PASTGA        = !down!
                    echo(
                    echo   PAUZA         = !pause!
                    echo(
                    echo   {Enter} - MENYUGA QAYTISH
                    echo(
call :ask ">KIRITING" !keys!
...
echo   ILON BOSHI      = !head!
echo   TANA QISMI      = !body!
echo   TAOM            = !food!
echo   HALOK BO'LISHI  = !death!
echo(
echo   O'SISH DARAJASI = !growth!
echo(
echo   {Enter} - MENYUGA QAYTISH
echo(
call :ask ">KIRITING" B E H S F D G {Enter}
if !key! equ {Enter} goto :saveUserPrefs
if /i !key! equ G (
...
cls
if defined replay (
  echo       TEZLIKNI TANLASH:
) else (
  echo        TEZLIKNI TANLANG:
)
                      
echo   T/r  QIYMAT TURI   HARAKAT 
echo   ---  -----------  ---------
for /l %%N in (1 1 6) do (
  set "delay=0!delay%%N!"
  set "desc=!desc%%N!           "
...
echo(
call :ask ".        TEZLIKNI TANLANG(1-6):" 1 2 3 4 5 6
set "difficulty=!desc%key%!"
set "delay=!delay%key%!"
set "diffCode=%key%"
echo %key% - %difficulty%
echo(
<nul set /p "=ILTIMOS KUTING."
set "axis=X"
set "xDiff=+1"
set "yDiff=+0"
set "empty="
...
if defined replay (
  ...
  echo(
  if !moveKeys! equ 4 (
    echo YO'NALISH: !up!=TEPA !down!=PAST !left!=CHAP !right!=O'NG !pause!=PAUZA
  ) else (
    echo YO'NALISH: !left!=CHAP !right!=O'NG !pause!=PAUZA
  )
  echo ILON O'ZINI O'ZI YEMASLIGI(!body!!body!!head!^) YOKI MAYDONGA TEGIB KETMASLIGI KERAK (!bound!^)
  echo(
  call :ask "DAVOM ETTIRISH...."
  %sendCmd% go
)
